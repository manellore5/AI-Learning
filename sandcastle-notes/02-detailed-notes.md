...
**Origin.** Sandcastle grew directly out of the limits observed in the two obvious ways to sandbox an autonomous agent. **Claude Code's built-in `/sandbox`** only wraps the bash tool — and the agent can break out of it, so it's unsafe for skip-permissions AFK runs. **Anthropic's Docker Sandbox** gives real microVM-grade isolation, but in practice it proved unstable and fast-changing, opaque (agent activity ran silently), and hard to parallelize or chain (implementer → reviewer). Sandcastle is the drop-in replacement that **solves exactly those problems**: real isolation you can trust, a shell-wrapped AFK harness collapsed to a ~17-line `main.ts`, the Ralph loop **built in as `maxIterations`**, easy parallelism and chaining, and full live logs (iteration count, sandbox setup, expanded shell expressions, every tool call). See [Recipe 5](#recipe-5--a-ralph-loop-autonomous-plan-runner) for the pattern.
