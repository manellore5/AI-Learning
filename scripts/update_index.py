...
def parse_note(path: Path) -> dict:
    """Return {title, url, added} — url/added may be None for plain notes."""
    ...
    return {"title": title, "url": url, "added": added, "path": path}
...
def write_master_index(sections: list[tuple[Path, list[dict]]]) -> None:
    ...
    for folder, notes in sorted(sections, key=lambda s: s[0].name.lower()):
        ...
        lines.append(f"| {link} | {len(notes)} | {last} |")
    if not sections:
